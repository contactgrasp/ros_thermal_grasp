...
def convert(input_filename, data_dir, output_dir):
  ...
  output_filename = osp.join(output_dir, '{:s}_macro.xacro'.format(robot_name))
  data_dir = osp.join(data_dir, robot_name)

  nsmap = {'xacro': 'http://wiki.ros.org/xacro'}
  robot = ET.Element('robot', nsmap=nsmap)
  hand = ET.SubElement(robot, '{%s}macro'%nsmap['xacro'],
    attrib={'name': robot_name, 'params': 'prefix'})
  ...
  def convert_link(g_link):
    ...
    mesh_filename = osp.join('package://ros_thermal_grasp', 'data', robot_name,
        '{:s}.stl'.format(link_name))
  ...
  for chain in g_hand.findall('chain'):
    ...
    for joint in chain.findall('joint'):
      ...
      link_name = '{:s}_fixed'.format(link_names[joint_idx])
      link = ET.Element('link', attrib={'name': link_name})
      hand.append(link)
  ...
  # add prefix to link and joint names
  for link in hand.iter('link'):
    link_name = '${prefix}' + link.get('name')
    link.set('name', link_name)
  for joint in hand.iter('joint'):
    joint_name = '${prefix}' + joint.get('name')
    joint.set('name', joint_name)
    p = joint.find('parent')
    p_name = '${prefix}' + p.get('link')
    p.set('link', p_name)
    c = joint.find('child')
    c_name = '${prefix}' + c.get('link')
    c.set('link', c_name)
